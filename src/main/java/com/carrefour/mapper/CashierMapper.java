...
public class CashierMapper {
	...
	private String formatDate(String formatDate, Date date) {
		SimpleDateFormat format = new SimpleDateFormat(formatDate);
		return format.format(date);
	}
	
	public cashier mapperEntityToCashier(Cashier cashierEntity, String werks) {
		cashier cashier = new cashier();
		
		cashier.setBadgecode(cashierEntity.getBadgecode());
		cashier.setCashcount(cashierEntity.getCashcount());
		cashier.setCashierId(cashierEntity.getCashier()); 
		...
		cashier.setUpdateDate( formatDate(Constant.FORMAT_UPDATE_DATE,calendar.getTime())); 
		cashier.setTraining(cashierEntity.getTraining());
		cashier.setStoreId(werks);
